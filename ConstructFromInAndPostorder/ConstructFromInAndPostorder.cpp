// ConstructFromInAndPostorder.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
Node* Construct(std::vector<int>& preorder, std::vector<int>& inorder, size_t& ptPre, size_t ptInStart, size_t ptInEnd)
{
    ...
    if (nodeIn + 1 < ptInEnd)
        pNode->right = Construct(preorder, inorder, --ptPre, nodeIn+1, ptInEnd);
    if (ptInStart < nodeIn)
...
{
    size_t ptPre = preorder.size() - 1, ptIn = 0;
